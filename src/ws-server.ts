...
async function handleSendMessage(ws: ExtendedWebSocket, payload: any) {
  ...
  try {
    ...
    // (In a real app, you'd get the member list from the API response)
    ...
    userConnections.forEach((connections, memberId) => {
...
// ==================== TYPING INDICATORS ====================
// Store typing timeouts to auto-clear typing state
  ...
  try {
    ...
    const response = await fetch(
      `${apiUrl}/api/chatroom/${chatRoomId}/messages/unread`,
      {
        ...
        body: JSON.stringify({
          userId,
          messageIds, // optional: specific messages, or all if undefined
        }),
      }
    );
